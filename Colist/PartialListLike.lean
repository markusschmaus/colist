...
structure equivExt {α : Type u} (x₁ : Data.Imp (PartialListLike α))
    (x₂ : Data.Imp (PartialListLike α)) : Prop where
  intro ::
  isNil_eq : x₁.inst.isNil x₁.value ↔ x₂.inst.isNil x₂.value
  head_heq : HEq (x₁.inst.head x₁.value) (x₂.inst.head x₂.value)

instance equivExt.instSetoid {α : Type u} : Setoid (Data.Imp <| PartialListLike α) where
  r := equivExt
  iseqv := by
    constructor
    · intro x
...
def equiv {α : Type u} (x₁ : Data.Imp (PartialListLike α))
    (x₂ : Data.Imp (PartialListLike α)) : Prop :=
  ∀ (n : ℕ), equivExt ⟨x₁.imp, x₁.inst, (x₁.inst.tail^[n] x₁.value)⟩ ⟨x₂.imp, x₂.inst, (x₂.inst.tail^[n] x₂.value)⟩

instance instSetoid (α : Type u) : Setoid (Data.Imp <| PartialListLike α) where
  r := equiv
  iseqv := by
    constructor
    · intro _ _
